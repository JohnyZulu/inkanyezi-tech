  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      armParticles.forEach(p => {
        ...
        const col = p.warm ? `rgba(255,210,150,${op})` : `rgba(200,220,255,${op})`;
...
function DoorAnimationInline({ onComplete }: { onComplete: () => void }) {
  ...
  const paintPanel = (canvas: HTMLCanvasElement, side: 'left'|'right', tk: number, op: number) => {
    ...
    const cols = [W*0.18,W*0.38,W*0.62,W*0.82];
    cols.forEach((sx,ci) => {
...
function InkanyeziBotWidget() {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onend = () => {
        setIsListening(prev => {
          if (prev) { setTimeout(() => { try { recognition.start(); } catch {} }, 100); return true; }
          return false;
        });
      };
  ...
  useEffect(() => {
    ...
    const doReset = () => {
      ...
      setInput(''); setShowLeadForm(false); setLeadFormSubmitted(false); setShowChips(true);
      setSessionContext(null); hasTriggered.current = false; sessionStorage.removeItem(STORAGE_KEY);
    };
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      let cleanMessage = (data.message || '').replace(/<context>[\s\S]*?<\/context>/gi, '').replace(/<response>|<\/response>/gi, '').replace(/\[THINK\][\s\S]*?\[\/THINK\]/gi, '').trim();
      const jIdx = cleanMessage.indexOf('\n{');
      if (jIdx > -1 && cleanMessage.includes('"pain_point"')) cleanMessage = cleanMessage.substring(0, jIdx).trim();
  ...
  const handleLeadSubmit = useCallback(async (formData: any) => {
    ...
    try {
      ...
      const payload = {
        ...
        reference_number: sessionContext?.referenceNumber||'', trigger_score: score,
        timestamp: new Date().toISOString(), sast_time: new Date().toLocaleString('en-ZA',{timeZone:'Africa/Johannesburg'}),
      };
      await fetch('https://hook.eu1.make.com/f4g89bwx1cor5euqad24pknn2iqbrmum', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) });
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes windowSlide { 0%{opacity:0;transform:translateY(30px) scaleY(0.05) scaleX(0.8);transform-origin:bottom center;} 40%{opacity:1;transform:translateY(0) scaleY(0.6) scaleX(1);transform-origin:bottom center;} 70%{transform:translateY(0) scaleY(1.02) scaleX(1);transform-origin:bottom center;} 100%{transform:translateY(0) scaleY(1) scaleX(1);transform-origin:bottom center;} }
        ...
        @supports (height: 100dvh) { .ink-widget-container { height: min(580px, calc(100dvh - 100px)) !important; } .ink-widget-container.ink-fullscreen { height: 100dvh !important; width: 100vw !important; bottom: 0 !important; right: 0 !important; border-radius: 0 !important; } }
        @supports (height: 100svh) { .ink-widget-container { height: min(580px, calc(100svh - 100px)) !important; } }
      ...
      {!isOpen && !showDoor && (
        <button onClick={()=>{ setShowDoor(true); setOpenKey(k=>k+1); }} aria-label="Open InkanyeziBot"
          style={{ position:'fixed', bottom:24, right:24, zIndex:99999, width:52, height:52, borderRadius:'50%', background:'linear-gradient(135deg, #FF6B35, #c2410c)', border:'2px solid rgba(249,115,22,0.45)', cursor:'pointer', fontSize:26, animation:'floatBubble 3s ease-in-out infinite', display:'flex', alignItems:'center', justifyContent:'center', transition:'background 0.3s', boxShadow:'0 0 30px rgba(249,115,22,0.55)' }}>
          ...
          <span style={{ position:'relative', zIndex:1, fontSize:24 }}>👨‍🚀</span>
        </button>
      )}

      {(showDoor || isOpen) && (
        <div className={`ink-widget-container${isFullscreen?' ink-fullscreen':''}`} style={{ position:'fixed', bottom:isFullscreen?0:82, right:isFullscreen?0:24, left:isFullscreen?0:'auto', top:isFullscreen?0:'auto', width:isFullscreen?'100vw':'min(370px, calc(100vw - 32px))', height:isFullscreen?'100vh':'min(580px, calc(100vh - 110px))', zIndex:99998, borderRadius:isFullscreen?0:20, overflow:'hidden', boxShadow:isFullscreen?'none':'0 0 0 1px rgba(244,185,66,0.15), 0 8px 40px rgba(0,0,0,0.25)', transition:'all 0.3s cubic-bezier(0.4,0,0.2,1)' }}>
          <div className="ink-widget-inner" style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', borderRadius:20, overflow:'hidden', background:'#FAFBFC' }}>
            <div style={{ position:'relative', zIndex:2, flexShrink:0, background:'linear-gradient(160deg, #0D1E35 0%, #0A1628 100%)', borderBottom:'1px solid rgba(0,229,255,0.15)', boxShadow:'0 1px 8px rgba(0,0,0,0.06)', padding:'12px 16px', display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div style={{ position:'relative', flexShrink:0 }}>
                <div style={{ width:42, height:42, borderRadius:'50%', background:'linear-gradient(135deg, #FF6B35, #c2410c)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, boxShadow:'0 0 16px rgba(249,115,22,0.6)' }}>👨‍🚀</div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                ...
                <button onClick={() => { setIsOpen(false); setIsFullscreen(false); }} title="Close chat"
            ...
            <div className="ink-msgs" style={{ flex:'1 1 0', minHeight:0, overflowY:'auto', padding:'14px 14px 6px', display:'flex', flexDirection:'column', gap:10, background:'transparent', position:'relative' }}>
              ...
              {messages.map((msg,i) => (
                <div key={i} className="ink-msg" style={{ display:'flex', justifyContent:msg.role==='user'?'flex-end':'flex-start', alignItems:'flex-end', gap:6, animationDelay:`${i*0.03}s` }}>
                  {msg.role==='assistant' && (
                    <div style={{ width:24, height:24, borderRadius:'50%', flexShrink:0, background:'linear-gradient(135deg, #FF6B35, #c2410c)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, boxShadow:'0 0 8px rgba(249,115,22,0.4)' }}>👨‍🚀</div>
                  )}

...
const translationCache = {}; // module-level cache: key = `${lang}_${industry}_${stageIdx}`
...
function DetailPanel({stageIdx,industry,lang,onNavigate}){
  ...
  useEffect(()=>{
    if(stageIdx<0||lang==="EN"){setTranslatedDesc(null);return;}
    const cacheKey=`${lang}_${industry}_${stageIdx}`;
    if(translationCache[cacheKey]){setTranslatedDesc(translationCache[cacheKey]);return;}
    const scenario=(IND[industry]||IND.General)[`s${stageIdx}`];
    const langName=LANGS.find(l=>l.code===lang)?.name||"English";
    setTranslating(true);
    setTranslatedDesc(null);
    const apiKey=typeof import.meta!=="undefined"&&import.meta.env?.VITE_GEMINI_API_KEY;
    if(!apiKey){setTranslating(false);return;}
    fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`,{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({contents:[{parts:[{text:`Translate the following South African business scenario into ${langName}. Keep it natural and conversational. Return only the translated text, nothing else.\n\n"${scenario.desc}"`}]}],generationConfig:{maxOutputTokens:200,temperature:0.3}})
    })
    .then(r=>r.json())
    .then(d=>{
      const text=d?.candidates?.[0]?.content?.parts?.[0]?.text?.trim()||"";
      if(text){translationCache[cacheKey]=text;setTranslatedDesc(text);}
    })
    .catch(()=>{})
    .finally(()=>setTranslating(false));
  },[stageIdx,industry,lang]);
  if(stageIdx<0){
    return(
  ...
  return(
    <div className="ink-detail" style={{borderRadius:12,overflow:"hidden",border:`1.5px solid ${node.color}40`}}>
      ...
      <div style={{background:"linear-gradient(135deg,#0d1e38,#0A1628)",padding:"16px 18px"}}>
        ...
        {/* SA Business scenario — always shows English industry-specific desc */}
        <div style={{background:"rgba(255,255,255,.04)",borderRadius:9,padding:"12px 14px",marginBottom:lang!=="EN"?8:12,borderLeft:`3px solid ${node.color}`}}>
        ...
        {/* Translated scenario — shown only when non-English language selected */}
        {lang!=="EN"&&(
          <div style={{background:"rgba(58,158,126,.06)",borderRadius:9,padding:"10px 14px",marginBottom:12,borderLeft:`3px solid ${C.teal}`}}>
            ...
            <p style={{fontSize:"clamp(11px,1.6vw,12px)",color:"rgba(250,246,238,.75)",lineHeight:1.75,margin:0,minHeight:36}}>
              {translating
                ? <span style={{opacity:.45,fontStyle:"italic"}}>Translating…</span>
                : (translatedDesc || <span style={{opacity:.45,fontStyle:"italic"}}>{langText}</span>)
              }
            </p>
          </div>
        )}
